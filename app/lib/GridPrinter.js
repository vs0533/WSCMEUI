 ...
 * columns
 * 
 * Modified by Loiane Groner - 2013-Feb-26 Fixed: added support for row expander
 * plugin Tested using Ext JS 4.1.2
 * 
 */
Ext.define("WsCme.lib.GridPrinter", {
	...
	statics : {
		...
		print : function(grid, featureId) {
			...
			items = items.concat(grid.normalGrid.getView().headerCt.items.items);
			Ext.each(items, function(c) {

						if (c.items.length > 0) {
							Ext.each(c.items.items, function(c1) {
										if (!c1.hidden)
											columns.push(c1);
									})
						} else {
			...
			grid.store.data.each(function(item, row) {
						...
						for (var key in item.data) {
							...
							Ext.each(columns, function(column, col) {
										...
										if (column && column.dataIndex == key) {
											...
											value = column.renderer
													? column.renderer.call(grid, value, meta, item, row,
															col, grid.store, grid.view)
													: value;
							...
							if (!found) { // model field not used on Grid Column, can be used
								// on RowExpander
			...
			if (this.stylesheetPath === null) {
				this.stylesheetPath = 'css/gridprint.css';
			}
			...
			if (pluginsBody != '') {
				pluginsBodyMarkup = [
						'<tr class="{[xindex % 2 === 0 ? "even" : "odd"]}"><td colspan="'
								+ columns.length + '">', pluginsBody, '</td></tr>'];
			}
			...
			var htmlMarkup = [
					...
					'<div class="buttons">',
					'<div class="' + Ext.baseCSSPrefix + 'ux-grid-printer-noprint '
							+ Ext.baseCSSPrefix + 'ux-grid-printer-links">',
					'<a class="'
							+ Ext.baseCSSPrefix
							+ 'ux-grid-printer-linkprint" href="javascript:void(0);" onclick="window.print();">'
							+ this.printLinkText + '</a>',
					'<a class="'
							+ Ext.baseCSSPrefix
							+ 'ux-grid-printer-linkclose" href="javascript:void(0);" onclick="window.close();">'
							+ this.closeLinkText + '</a>', '<hr/></div>', '</div>',
					'<div class="title">' + this.mainTitle + '</div>', '<table>', '<tr>',
					headings, '</tr>', '<tpl for=".">',
					'<tr class="{[xindex % 2 === 0 ? "even" : "odd"]}">', body, '</tr>',
					pluginsBodyMarkup.join(''),
					'{% if (this.isGrouped && xindex > 1) break; %}', '</tpl>',
					'</table>', '</body>', '</html>', {
						isGrouped : isGrouped
					}];
		...
		generateBody : function(grid, columns, feature) {
			...
			if (grid.store.isGrouped()) {
				...
				var bodyTpl = [
						...
						' </tr>', '<tpl for="children">', '<tr>',
						'<tpl for="this.fields">',
						'{% if (values.name==="id") continue; %}', '<td>',
						'{[ parent.get(values.name) ]}', '</td>', '</tpl>', '</tr>',
						'</tpl>', '</tpl>', {
		 ...
		 * @type String Title to be used if grid to be printed has no title
		 *       attribute set.
		 */
		...
		/**
		 * Text shown as postfix to group header text, if there is only one child
		...
		/**
		 * Text shown as postfix to group header text, if there are multiple child
		 ...
		 */
		bodyTpl : ['<tpl for=".">', '<tpl if="values.dataIndex">',
				'<td>\{{[Ext.String.createVarName(values.dataIndex)]}\}</td>',
				'<tpl else>', '<td>\{{[Ext.String.createVarName(values.id)]}\}</td>',
				'</tpl>', '</tpl>']
	}
});